...
def maintenance_inference(
    ...
    print(f"[Maintenance Inference] Ran detection on frame {frame_i}: {updated_count}/{len(features)} features updated.")
...
def find_best_starting_frame(frames: list, model_confidence=0.5, attempts: int = 50, show: bool = False) -> tuple[int, object]:
    ...
    print("Finding the best starting frame")
    ...
    print(f"Best starting frame found: Frame {best_frame['frame_index']} with a combined ear+head confidence of {round(best_frame['confidence_sum'],4)} - Found {len(best_frame["detected_objects"])} features")
    if show:
